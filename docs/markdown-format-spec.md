...
### 7.5 拡張フォーマット（プロジェクトコード先頭配置）

スケジュール形式と同様に、プロジェクトコードをチェックボックスの直後に配置することも可能です：

```markdown
- [ ] [P99] タスク名
- [ ] @2025-12-31 [P99] 期限付きタスク
- [ ] [P99] !!! 高優先度タスク
```
...
```javascript
// TODO項目検出（2パターン対応）
// パターン1: - [ ] @日付 優先度 タスク名（従来形式）
// パターン2: - [ ] [PXX] タスク名（プロジェクトコード先頭）
const todoItemPattern = /^-\s+\[([xX\s\*\-])\]\s+(.+)$/;

// プロジェクトコード抽出（先頭）- 新フォーマット対応
...
**パース順序**（プロジェクトコードが先頭にある場合）：
1. ステータス記号を抽出
2. プロジェクトコード `[PXX]` を抽出（あれば）
3. 期日 `@YYYY-MM-DD` を抽出（あれば）
4. 優先度 `!!!` `!!` `!` を抽出（あれば）
5. 残りをタスク名として処理
...
### [Mon]
* 08:00 [P99] タスク確認・整理、日報返信
* 09:00 [P99] 雑賀さんBootCamp
* 09:30 [P37] kao 花王デイリー
* 10:00 [P34] dhc 基盤相談会
* 17:00 [P34] dhc SREリファインメント
- [ ] [P99] 週次レポート作成

### [Tue]
* 08:00 [P99] タスク確認・整理、日報返信
* 09:30 [P99] 経営会議
* 12:45 [904] LLM-Admin MTG
* 14:00 [P37] 花王MTG

### [Wed]
* 08:00 [P99] タスク確認・整理、日報返信
* 14:00 [P34] dhc 統合スプリントレビュー
...
### [Fri]
* 08:00 [P99] タスク確認・整理、日報返信
* 14:00 [P25] 東大三井 社内MTG

### [adhoc]
* 14:00 [P14] SuGuSuPa 開発MTG
